// Magic-mode toggle.
//
// Adds a small fixed wand button that turns the magic stick + dark-room spotlight
// effect on or off. State persists in localStorage as `magic-mode` = 'on' | 'off'.
// When OFF, an `.magic-off` class on <html> hides the wand SVG, the trail canvas
// and the spotlight overlay, and restores the native cursor (cursor.ts uses
// `:not(.magic-off)` guards so its `cursor: none !important` rule no longer
// applies).
//
// On touch devices and prefers-reduced-motion the underlying effects are already
// disabled by their own gates — we still render the toggle so the user has a
// visible affordance, but it just shows "magic mode unavailable" tooltip.
...
(function init() {
  ...
  // Read persisted state. Default OFF — magic mode is opt-in.
  function readState(): "on" | "off" {
    const v = localStorage.getItem(STORAGE_KEY);
    if (v === "on" || v === "off") return v;
    return "off";
  }
  // `supported` is referenced only inside `applyState` for tooltip text.
  void supported;
  ...
  function applyState(state: "on" | "off") {
    ...
    try {
      ...
      // localStorage may be disabled in private mode — toggle still works in-session
    }
    if (btn) {
  ...
  // Inject shared CSS that drives the off-state visibility + button styling.
  ...
  // Build the button + SVG via DOM APIs (no innerHTML).
  const NS = "http://www.w3.org/2000/svg";
  const btn = document.createElement("button");
  btn.setAttribute("type", "button");
  btn.setAttribute("data-magic-toggle", "");
  btn.setAttribute("data-keep-cursor", ""); // ensure native cursor on the button itself even when magic-on
  ...
  // Apply initial state IMMEDIATELY (before children render) by toggling the class
  // on <html>, then attach the button to the body. Keeps the page from flashing
  // the magic effect for a frame when state is OFF.
  document.documentElement.classList.toggle("magic-off", readState() === "off");

  function attach() {
    // Replace the spinner placeholder rendered eagerly by boot.ts (if present)
