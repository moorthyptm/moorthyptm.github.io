...
// Build-time fetch from the agent — no local fallback. Dev defaults to
// http://localhost:8787; CI hits the deployed endpoint.
import {
  renderExperience,
  experienceIntro,
  renderCommunity,
...
async function loadDataFile<S extends v.GenericSchema>(
  ...
  try {
    ...
    // Network blip → fall back to whatever we cached, if anything.
    if (cache) {
      // eslint-disable-next-line no-console
...
// Build a structured context the templates resolve against. Computed
// conveniences (firstName, locationCity, focusAreas etc.) sit alongside the
// raw profile fields so templates use one path per value.
...
// Mustache-style resolver. {{a.b.c}} walks the context; missing paths
// resolve to the empty string (the build-time leak guard catches typos).
...
export default defineConfig(async ({ command }) => {
  ...
  // Mutable so the dev-mode HMR plugin can refresh content without restart.
  ...
  return {
    ...
    plugins: [
      tailwindcss(),
      {
        // Dev-only: poll the agent for JSON changes; on a fresh ETag, re-fetch
        // + rebuild ctx + full-reload the browser. Agent restarts on JSON
        // edits via tsx watch (JSON is imported as ESM module).
        name: "agent-data-hmr",
        apply: "serve",
      ...
      },
      {
        ...
        // `pre` runs before Vite's HTML pipeline so downstream parsers see
        // already-substituted content (CSP / JSON-LD / meta).
        transformIndexHtml: {
          order: "pre",
      ...
      {
        // Build-only: strict CSP. Replace 'unsafe-inline' in script-src with
        // sha256 hashes of every inline script (JSON-LD + src=external are
        // skipped). Strip 'unsafe-inline' from style-src — the prod build
        // emits no inline styles. Dev keeps 'unsafe-inline' because Tailwind
        // / Vite HMR inject style tags dynamically at runtime.
        name: "csp-tighten",
        apply: "build",
        transformIndexHtml: {
          order: "post",
